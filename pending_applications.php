<?php 
 ...
 if(!$isAdmin) {
  ...
  header("Location: index.php?alert=You need to login to do that.");
 }
...
<?php  
  $db = new Database;
  $charities = $db->getUsersPendingCharities($userId);
  if(isset($_GET['msg'])) {
...
<div class="block">
  ...
  <?php 
  if(!$charities) {
    echo('<div class="block"> No pending charities to display </div>');
    exit;
  }
  ?>
